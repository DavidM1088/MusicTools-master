...
class Chord  : StaffObject {
    ...
    func sortNotes() {
        ...
        //let sortedNotes : [Note] = notes.sort(by: { (s1: Note, s2: Note) -> Bool in
        //    return s1.noteValue < s2.noteValue
        //})
        
        self.notes.sort {
            $0.noteValue < $1.noteValue
        }
    ...
    class func voiceLead(chord1 : Chord, chord2 : Chord) -> Chord {
        ...
        var candidate : Chord = Chord.incrementChord(chord: chord2, incr: -24)
        while ctr < 4 * chord1.notes.count {
            candidates.append(candidate)
            candidateDiffs.append(Chord.chordDifference(chord1: candidate, chord2: chord1))
            print("cand \(candidate.toString()) diff: \(candidateDiffs[ctr])")
            candidate = Chord.invert(chord: candidate)
            ctr += 1
        }
        //return the transpose with the lowest score (i.e. closest to first chord)
        var lowestDiff = 9999
        var lowest = 0
        let hiLo: Int = Int(arc4random()) % 2
    ...
    class func expandChord(chordIn : Chord, chordSize : Int, lo: Int, hi : Int) -> Chord {
        ...
        while notes.count < chordSize {
            ...
            let octave = Int(arc4random()) % 2
            let noteIndex = Int(arc4random()) % baseNotes.count
    ...
    func toString() -> String {
        ...
        for note in self.notes {
            res = res + "  \(Note.noteDescription(note: note, withOctave: true)) \(note.noteValue), "
        }
        return res
    }
