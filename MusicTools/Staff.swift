...
class Voice {
    ...
    func putOnStaff(chord : Chord) -> Chord {
        ...
        if (self.clef == CLEF_TREBLE) {
            if (hiNote > MIDDLE_C + 2 * OCTAVE_OFFSET - 3) {
                newChord = Chord.incrementChord(chord: newChord, incr: -OCTAVE_OFFSET)
            }
            if (loNote < MIDDLE_C) {
                newChord = Chord.incrementChord(chord: newChord, incr: +OCTAVE_OFFSET)
            }
    ...
    func play() {
        ...
        if self.lastSound != nil {
            ...
            //}
            instrument.stop(object: lastSound!)
            self.lastSound = nil
        }
        ...
        //}
        instrument.play(object: contents[beatNum])
...
class Staff {
    ...
    deinit {
        //Logger.log(message: "Staff deinit")
    }
    ...
    @objc func beat() {
        ...
        if done || stopForced {
            ...
            self.timer = Timer.scheduledTimer(timeInterval: self.tempo, target: self, selector: #selector(Staff.beat), userInfo: nil, repeats: false)
        }
    }
